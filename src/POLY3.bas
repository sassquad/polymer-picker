...
ENDPROC
DEFPROCps
MOVEOSX%,OSY%:VDU18,3,2,192,193,194,195,8,8,8,8,10,196,197,198,199
IFSX%<=X% PROCsr ELSE PROCsl
MOVESX%,SY%:VDU18,3,2,192,193,194,195,8,8,8,8,10,196,197,198,199
ENDPROC
DEFPROCmovefish
...
PROCair
A%=X%:B%=Y%:C%=Z%
IFINKEY(-(?&100+1))ANDX%>0:X%=X%-(DF%*32):Z%=-1
IFINKEY(-(?&101+1))ANDX%<1080:X%=X%+(DF%*32):Z%=1
IFINKEY(-(?&102+1))ANDY%<864:Y%=Y%+(DF%*16)
IFINKEY(-(?&103+1))ANDY%>224:Y%=Y%-(DF%*16)
IFINKEY(-(?&104+1))AND(X%<>A% OR Y%<>B%):DF%=2:OD%=100:PROCair:ELSE:DF%=1:OD%=200
IFX%<>A% OR Y%<>B% MOVEA%,B%:PROCP(C%):PROCb(A%,B%):MOVEX%,Y%:PROCP(Z%):PROCb(X%,Y%)
CX%=X%+88
CY%=Y%-56
IFPOINT(CX%,CY%)=1:PROCcheck(CX%,CY%)
IFPOINT(CX%,CY%)=5 OD%=50:SF%=2:PROChurt ELSE OD%=200
ENDPROC
DEFPROChurt
...
TIME=0:REPEATUNTILTIME>300:VDU5
ENDPROC
DEFPROCover
VDU4,31,5,2
